...
# --- SETTINGS ---
OUTPUT_FILE = "DataML/podium_prediction_2025_japan_normalized.csv"
YEAR_PREDICT = 2025
ROUND = 3  # Japanese GP (Round 3)
CIRCUIT_ID = 22  # Replace with the correct ID for Suzuka from circuits_full.csv
...
# --- STEP 1: Load 2024 results (for historical form and circuit averages) ---
results_list = []
    ...
    try:
        ...
        for row in session.results.itertuples():
            results_list.append({
                ...
                "circuitId": event["EventName"]  # Using name for filtering below
            })
...
# --- STEP 2: Fetch 2025 Qualifying data for Japanese GP ---
...
for row in session_2025.results.itertuples():
    grid_data.append({
        ...
        "constructorId": row.TeamName.lower().replace(" ", "_"),
        "grid": row.Position,
        "driverStandPos": 20.0,   # We can adjust later if points are available after China
        "driverPoints": 0.0,
...
# --- STEP 3: Calculate form and historical averages (Suzuka GP) ---
for idx, row in df_2025.iterrows():
    ...
    # Historical averages at Suzuka (filter by circuit name containing "Japan")
    drv_races = results_df[(results_df["driverId"] == drv) & (results_df["circuitId"].str.contains("Japan", case=False))]
    df_2025.loc[idx, "driverAvgCircuit"] = drv_races["positionOrder"].mean() if not drv_races.empty else 10.0

    cons_races = results_df[(results_df["constructorId"] == cons) & (results_df["circuitId"].str.contains("Japan", case=False))]
    df_2025.loc[idx, "constructorAvgCircuit"] = cons_races["positionOrder"].mean() if not cons_races.empty else 10.0

# --- STEP 4: Normalize features to match training scale (0–1) ---
for col in ["grid", "driverStandPos", "driverPoints", "constructorStandPos",
    ...
    df_2025[col] = min_max_scale(df_2025[col], min_val=0, max_val=25)  # Assumes 0–25 range scaling

# --- SAVE the final normalized dataset ---
df_2025.to_csv(OUTPUT_FILE, sep=";", index=False)
print(f"Normalized CSV for 2025 Japanese GP created: {OUTPUT_FILE}")
